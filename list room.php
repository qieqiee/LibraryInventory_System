...
if (is_array($roomIds) && !empty($roomIds)) {
    ...
    if ($length == 0) {
        echo "All rooms are fully booked";
    } else {
        echo "Rooms available";
    }
} else {
    echo "No room information available";
}
...
<nav class="nav nav-3">
    ...
    <div class="main">
        ...
        if(isset($_POST['book'])){
          $roomid=$_POST['roomid'];
          $sql2 = "INSERT INTO `roombook_stud` (`bookingid`, `date`, `checkin_time`, `checkout_time`, `studNo`, `roomid`) VALUES (NULL, '$date', '$checkin', '$checkout', '$studNo', '$roomid')";


          $result2 = mysqli_query($conn, $sql2);
          echo "<script>alert('Room successfully booked');</script>";
          echo "<script>window.location.href='room form.php';</script>";
        }
